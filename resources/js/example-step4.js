		...
		FB.login(function(loginResponse) {
			...
			if (loginResponse.status === "connected" && loginResponse.authResponse) {
				...
				FB.api("/me", function(profileResponse) {
					...
					/*
					 * Announce/publish that we have liftoff
					 */
					__this.publish("login.facebook.complete", {
	...
	var
		...
		__initializeFacebook = function() {
			...
			window.fbAsyncInit = function() {
				...
				/*
				 * Do a status call which executes after Facebook is 
				 * initialized so we can publish a message stating
				 * we done initializing.
				 */
				FB.getLoginStatus(function(response) {
					__this.publish("init.facebook.complete", {});
				});
			};
...
/*
 * Give us observer pattern
 */
YAOF.attach(CFMLGroup.Account.LoginService);
